...
// Controlador que elimina un gasto por su ID (DELETE /transacciones/gasto/:id)
// Parametros de ruta: id (numero) - identificador del gasto a eliminar
...
export const eliminarGasto = async (req: Request, res: Response) => {
  ...
  try {
    // Ejecutamos la consulta SQL para eliminar el gasto
    ...
    const consulta = await conexion.execute({
      sql: "DELETE FROM gastos WHERE id = ? AND idUsuario = ?",
      args: [id, usuario.id],
    });
