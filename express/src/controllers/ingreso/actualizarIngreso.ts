...
// Controlador que actualiza un ingreso existente por su ID (PUT /transacciones/ingreso/:id)
// Parametros de ruta: id (numero) - identificador del ingreso a actualizar
// Parametros del cuerpo: monto (numero), descripcion (string opcional), fecha (string)
...
export const actualizarIngreso = async (req: Request, res: Response) => {
  ...
  try {
    // Ejecutamos la consulta SQL para actualizar el ingreso
    ...
    const consulta = await conexion.execute({
      sql: "UPDATE ingresos SET monto = ?, descripcion = ?, fecha = ? WHERE id = ? AND idUsuario = ?",
      args: [monto, descripcion || null, fecha, id, usuario.id],
    });
